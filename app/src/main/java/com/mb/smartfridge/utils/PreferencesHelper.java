...
public class PreferencesHelper {
	...
	public static PreferencesHelper getInstance(String sharedPreferencesName){
		...
		if (sInstance.mSharePreferences==null || (TextUtils.isEmpty(sharedPreferencesName) && TextUtils.isEmpty(sLastSharedPreferencesName))){
			useDefault = TextUtils.isEmpty(sharedPreferencesName);
		}else if ( !sharedPreferencesName.equals(sLastSharedPreferencesName)){
			useDefault = TextUtils.isEmpty(sharedPreferencesName);
		}else{
			return sInstance;
		}
		...
		if (useDefault){
			sInstance.mSharePreferences = PreferenceManager.getDefaultSharedPreferences(MyApplication.getAppContext());
			sLastSharedPreferencesName = null;
		}else{
			sInstance.mSharePreferences = MyApplication.getAppContext().getSharedPreferences(sharedPreferencesName, Context.MODE_PRIVATE);
